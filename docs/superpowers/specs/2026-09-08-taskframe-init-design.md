...
- Flutter 3.x
- Таргеты: Android, Web. iOS — платформенная папка создаётся, но сборка и
  запуск не проверяются на этом этапе (нет macOS/Xcode). iOS CI добавится
  позже отдельно на macOS-раннере (Codemagic / GitHub Actions).
...
- `flutter run` работает на Android-эмуляторе и в браузере (Chrome).
- Экран `DayScreen` рендерится одинаково на обеих платформах.
- Переключение системной темы меняет оформление приложения.
